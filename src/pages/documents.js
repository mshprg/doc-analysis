...
import {createDocumentChat, getAllDocuments} from "@/http/api/chatAPI";
...
function Documents() {
    ...
    useEffect(() => {
        ...
        setChats([])
        setLoading(false)
        getAllDocuments(user_id).then(data => {
    ...
    const handleFileChange = async (e) => {
        ...
        formData.append('isPractice', isPractice ? '1' : '0')

        const data = await createDocumentChat(formData)
    ...
    const changePopupVisible = (value) => {
        setPopupVisible(value)
    }

    const changePractice = (value) => {
        setIsPractice(value)
    }
    ...
    return (
        <>
            <Popup
                ...
                setVisible={(value) => changePopupVisible(value)}
                setIsPractice={(value) => changePractice(value)}
                clickFileRef={clickFileRef}
            />
            <HeightWrapper dir="row">
                <div className={styles.wrap}>
                    <h2 className={styles.h2_t}>Ваши документы</h2>
